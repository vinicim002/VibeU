...
export function DashboardLayout({ title, navItems }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-bg-dark">
      <header className="border-b border-white/5 bg-bg-slate/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <Link to={ROUTES.HOME} className="font-display text-2xl tracking-widest text-white">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        >
          <h1 className="font-display text-4xl tracking-wider text-white md:text-5xl">
            {title}
          </h1>
          <p className="mt-2 text-text-muted">Painel de controle — {user?.role.toLowerCase()}</p>
        </motion.div>

        <nav className="mt-8 flex gap-2 overflow-x-auto border-b border-white/5 pb-4">
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
