    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to={ROUTES.HOME} className="group flex items-center gap-2">
          <span className="font-display text-2xl tracking-widest text-foreground transition group-hover:text-primary-light">
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <>
              <Link
                to={dashboardRoute}
                className="hidden text-xs font-semibold uppercase tracking-wider text-text-secondary hover:text-foreground sm:block"
              >
                {user.name.split(' ')[0]}
              </Link>
